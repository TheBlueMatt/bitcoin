...
ChunkCodedBlock::ChunkCodedBlock(const CBlock& block, const CBlockHeaderAndLengthShortTxIDs& headerAndIDs) {
    ...
    // Append the block header at the end. We dont currently use this in decode,
    // but this should allow us to decode a block without ever having fully
    // received the header-and-short-ids.
