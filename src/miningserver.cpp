...
struct ClientState {
    ...
    ClientState() : m_client_id(max_client_id.fetch_add(1)), m_msg_read_buffer(7) {} // first message is PROTOCOL_SUPPORT, of 6 bytes + message type byte
};
...
std::vector<unsigned char> LibeventContext::EncodeBestTemplateHeader(uint64_t client_id) {
    ...
    std::vector<unsigned char> encoded_template(65);
    encoded_template[0] = (uint8_t)MsgTypes::BLOCK_TEMPLATE_HEADER;
    CVectorWriter writer(SER_NETWORK, PROTOCOL_VERSION, encoded_template, 65);

    writer << block_template.first;
    writer << client_id;
    writer << ArithToUint256(arith_uint256().SetCompact(block_template.second.m_block->nBits));

    block_template.second.m_block->vtx[0] = BuildHeaderVariantCoinbaseTx(block_template.second.m_original_coinbase_tx, client_id);

    writer << block_template.second.m_original_version;
    writer << block_template.second.m_block->hashPrevBlock;
    writer << BlockMerkleRoot(*block_template.second.m_block);
    ...
    CSHA256()
        .Write(&type, 1)
        .Write(encoded_template.data() + 65, encoded_template.size() - 65)
        .Finalize(message_hash.begin());
    assert(m_auth_key.SignNonRecoverableCompact(message_hash, encoded_template.data() + 1));
...
std::vector<unsigned char> LibeventContext::EncodeBestTemplate(bool need_full_payout) {
    ...
    std::vector<uint256> merkle_rhss = BlockMerkleBranch(*block_template.second.m_block, 0);
    writer << (uint8_t) merkle_rhss.size();
    for (const uint256& hash : merkle_rhss) {
        writer << hash;
    }
        ...
        if (client->m_next_msg_type == MsgTypes::UNKNOWN) {
            ...
            switch(client->m_next_msg_type) {
                case MsgTypes::WINNING_NONCE: {
                    client->m_msg_read_buffer.resize(21);
                    client->m_next_msg_offset = 0;
                    break;
                }
                case MsgTypes::TRANSACTION_DATA_REQUEST: {
                    client->m_msg_read_buffer.resize(8);
                    client->m_next_msg_offset = 0;
                    break;
                }
                case MsgTypes::WINNING_NONCE_HEADER: {
                    client->m_msg_read_buffer.resize(29);
                    client->m_next_msg_offset = 0;
                    break;
                }
                default: {
            ...
            if (client->m_next_msg_offset == client->m_msg_read_buffer.size()) {
                switch(client->m_next_msg_type) {
                    case MsgTypes::PROTOCOL_SUPPORT: {
                        ...
                        uint8_t message_type_byte;
                        uint16_t min_protocol_version, max_protocol_version;
                        VectorReader reader(client->m_msg_read_buffer, SER_NETWORK, PROTOCOL_VERSION);
                        reader >> message_type_byte >> min_protocol_version >> max_protocol_version >> client->m_flags;

                        if (message_type_byte != 1 || min_protocol_version > CURRENT_MINING_PROTOCOL_VERSION || max_protocol_version < CURRENT_MINING_PROTOCOL_VERSION) {
                            return ctx->DisconnectAndFreeClient(client_it);
                        }
                        ...
                        if (!(client->m_flags & (uint16_t)VersionFlags::NEED_HEADER_TEMPLATE_VARIANTS)) {
                            client->m_outbound_messages_pending.emplace_back(65);
                            std::vector<unsigned char>& msg = client->m_outbound_messages_pending.back();
                            msg[0] = (uint8_t)MsgTypes::COINBASE_PREFIX_POSTFIX;
                            CVectorWriter writer(SER_NETWORK, PROTOCOL_VERSION, client->m_outbound_messages_pending.back(), 65);
                            ...
                            CSHA256()
                                .Write(&type, 1)
                                .Write(msg.data() + 65, msg.size() - 65)
                                .Finalize(message_hash.begin());
                            assert(ctx->m_auth_key.SignNonRecoverableCompact(message_hash, msg.data() + 1));
                        }
                    ...
                    case MsgTypes::WINNING_NONCE: {
                        ...
                        if (client->m_msg_read_buffer.size() == 21) {
                            client->m_msg_read_buffer.resize(21 + user_tag_len + 4);
                        } else if (client->m_msg_read_buffer.size() == (size_t)21 + user_tag_len + 4) {
                            reader.seek(user_tag_len);
                            uint32_t coinbase_tx_len;
                            reader >> coinbase_tx_len;

                            client->m_msg_read_buffer.resize(21 + user_tag_len + 4 + coinbase_tx_len);
                        } else {
                            ...
                            template_it->second.m_block->hashMerkleRoot = BlockMerkleRoot(*template_it->second.m_block);
                    ...
                    case MsgTypes::TRANSACTION_DATA_REQUEST: {
                        ...
                        client->m_outbound_messages_pending.emplace_back(65);
                        client->m_outbound_messages_pending.back()[0] = (uint8_t)MsgTypes::TRANSACTION_DATA;
                        CVectorWriter writer(SER_NETWORK, PROTOCOL_VERSION, client->m_outbound_messages_pending.back(), 65);

                        std::vector<unsigned char> tx_serialize_buff;

                        uint32_t tx_count = template_it->second.m_block->vtx.size() - 1;
                        writer << template_id << tx_count;
                        ...
                        CSHA256()
                            .Write(&msg_type, 1)
                            .Write(client->m_outbound_messages_pending.back().data() + 65, client->m_outbound_messages_pending.back().size() - 65)
                            .Finalize(message_hash.begin());
                        assert(ctx->m_auth_key.SignNonRecoverableCompact(message_hash, client->m_outbound_messages_pending.back().data() + 1));
                    ...
                    case MsgTypes::WINNING_NONCE_HEADER: {
                        ...
                        if (client->m_msg_read_buffer.size() == 29) {
                            client->m_msg_read_buffer.resize(29 + user_tag_len);
                        } else {
