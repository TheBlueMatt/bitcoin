...
macro_rules! decoder_fn {
    ($name:ident, $val_type:ty, $readfn:ident) => {
        ...
        fn $name(&mut self) -> Result<$val_type, Error> {
            let mut val = [0; std::mem::size_of::<$val_type>()];
...
impl<R: Read> ReadExt for R {
    decoder_fn!(read_u64, u64, slice_to_u64_le);
    decoder_fn!(read_u32, u32, slice_to_u32_le);
    decoder_fn!(read_u16, u16, slice_to_u16_le);
    decoder_fn!(read_i64, i64, slice_to_i64_le);
    decoder_fn!(read_i32, i32, slice_to_i32_le);
    decoder_fn!(read_i16, i16, slice_to_i16_le);
