...
use crate::TOTAL_MEM_LIMIT_BYTES;
...
struct PeerState {
    ...
    /// If this peer provided a block that we didn't get from any other peers, we set this flag
    /// and almost certainly keep the connection alive.
    provided_rare_block: bool,
}

/// We very tightly limit the maximum peer count to avoid using more than our memory limit.
/// We calculate it based on our memory limit and the maximum "real" bytes in the message queue.
/// Note that this may slightly violate the memory limit, so we subtract 4 for a bit of headroom.
/// Currently, this translates to a 29 peer limit.
const PEER_LIMIT: usize = (TOTAL_MEM_LIMIT_BYTES / bitcoin::consensus::encode::MAX_VEC_SIZE / MAX_QUEUE_LEN / 2) - 4;
/// If we want to create a new outbound connection, and we have fewer than this many outbound
/// peers, we first disconnect inbound peers.
const RESERVED_OUTBOUND: usize = PEER_LIMIT / 2;
            ...
            'aggressiveness_check: while unsafe { !rusty_ShutdownRequested() } {
                ...
                if prev_tip == new_tip && new_tip != new_header_tip &&
                    ...
                    if let Some(addr) = addr_tracker.get_rand_addr(&mut rand_ctx) {
                        log_line(&format!("Progress not being made, connecting to {:?}...", addr), false);
                        if peers.len() >= PEER_LIMIT {
                            let mut outbounds = 0;
                            for peer in peers.iter() {
                                if peer.is_outbound { outbounds += 1; }
                            }
                            peers.sort_unstable_by(|a, b| {
                                if outbounds < RESERVED_OUTBOUND {
                                    if a.is_outbound && !b.is_outbound {
                                        return cmp::Ordering::Greater;
                                    } else if !a.is_outbound && b.is_outbound {
                                        return cmp::Ordering::Less;
                                    }
                                }
                                if a.provided_rare_block && !b.provided_rare_block {
                                    return cmp::Ordering::Greater;
                                } else if !a.provided_rare_block && b.provided_rare_block {
                                    return cmp::Ordering::Less;
                                }
                                b.connect_time.cmp(&a.connect_time)
                            });
                            log_line(&format!("Disconnecting from {:?} to make new outbound connection.", peers[0].remote_addr), false);
                            peers[0].queues.outbound.lock().unwrap().push_back(NetMsg::EOF);
                            peers.remove(0);
                        }
...

                {
                    ...
                    for (queues, addr) in inbounds.drain(..) {
                        if peers.len() >= PEER_LIMIT {
                            // TODO: We should, at least sometimes, prefer to evict a peer instead
                            // of rejecting new connections.
                            queues.outbound.lock().unwrap().push_back(NetMsg::EOF);
                        } else {
                            peers.push(PeerState {
                                queues,
                                remote_addr: addr,
                                ...
                                NetMsg::SerializedBlock(blockdata) => {
                                    ...
                                    if let Ok(header) = BlockHeader::consensus_decode(&blockdata[..]) {
                                        if let Some(index) = BlockIndex::get_from_hash(&header.bitcoin_hash().into_inner(), Some(new_tip)) {
                                            let had_block = index.have_block();
                                            if !had_block && is_probably_on_best_chain(new_tip, index, peer.block_state.get_current_best()) {
                                                connect_block(&blockdata, Some(index));
                                            } else if !had_block {
                                                connect_block(&blockdata, None);
                                            }
                                            if !had_block && peer.next_block_request == Some(index) && index.have_block() {
                                                // We requested this block, and the peer gave it to
                                                // us, mark them has a great and helpful peer.
                                                peer.provided_rare_block = true;
                                            }
                                        } else {
